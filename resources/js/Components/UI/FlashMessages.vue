<template>
    <div>
        <div v-if="$page.flash.success && show"
             class="mb-8 flex items-center justify-between bg-green-500 rounded w-full">
        ...
        <div v-if="$page.flash.status && show"
             class="mb-8 flex items-center justify-between bg-indigo-500 rounded w-full">
            <div class="flex items-center">
                <svg class="ml-4 mr-2 flex-shrink-0 w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg"
                     viewBox="0 0 20 20">
                    <polygon points="0 11 2 9 7 14 18 3 20 5 7 18"/>
                </svg>
                <div class="py-4 text-white text-sm font-medium" v-if="$page.flash.status === 'verification-link-sent'">
                    Se ha enviado un nuevo enlace de verificación a la dirección de correo electrónico que proporcionó
                    durante el registro.
                </div>
                <div class="py-4 text-white text-sm font-medium" v-else>
                    {{ $page.flash.status }}
                </div>
            </div>
            <button type="button" class="group mr-2 p-2" @click="show = false">
        ...
        <div v-if="($page.flash.error || Object.keys($page.errors).length > 0) && show"
            ...
            <div class="flex items-center">
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    <span v-if="Object.keys($page.errors).length === 1">Tienes 1 error en el formulario</span>
                    <span v-else>Tienes {{ Object.keys($page.errors).length }} errores en el formulario</span>
                </div>
            </div>
            <button type="button" class="group mr-2 p-2" @click="show = false">
